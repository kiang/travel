...
class GeocodableBehavior extends ModelBehavior {
	...
	public $default = array(
		'service' => 'google',
		'key' => 'ABQIAAAApmhX0DdZESsry567YeF_vBQsyKWKatEedhTiXOAzs3CgIFNTaRRslrhN7aHTAhCwdYJX3j9Tu9mXIA',
		'fields' => array(
			'hash' => false,
			'address',
			'latitude',
	...
	protected $services = array(
		'google' => array(
			'url' => 'http://maps.google.com/maps/geo?q=${address}&key=${key}&hl=en_US',
	...
	protected function _fetchCoordinates($settings, $address) {
		...
		$this->result = json_decode($this->socket->get($url));

		if (empty($this->result) || !isset($this->result->Status->code) ||
		    $this->result->Status->code != '200' ||
		    empty($this->result->Placemark[0]->Point->coordinates)
		) {
			return false;
		} else {
		    return array(
		        $this->result->Placemark[0]->Point->coordinates[0], //longitude
		        $this->result->Placemark[0]->Point->coordinates[1]  //latitude
		    );
		}
	}
