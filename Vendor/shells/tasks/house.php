<?php
class HouseTask extends HotelShell {
    var $baseUrl = 'http://202.39.225.136/hotel/h_house.asp?SYS_PageSize=%u&PageNo=%u';
    var $key = '<td width="70"><img src="run.JPG" border="0">';
    var $dataFile = 'house.html';
    var $type_id = 4;
	function execute() {
	    parent::initialize();
	    $this->extractData();
	...
	function extractData() {
	    ...
	    for($i = 3; $i <= $count; $i++) {
	        ...
	        $data['Hotel']['rooms'] = $this->trimTags($arr1[0][6]);
	        $data['Hotel']['phone'] = $this->trimTags($arr1[0][4]);
	        $data['Hotel']['fax'] = $this->trimTags($arr2[0][4]);
	        $data['Hotel']['address'] = $this->trimTags($arr1[0][5]);
	        $data['Hotel']['eaddress'] = $this->trimTags($arr2[0][5]);
	        $price = explode('-', $this->trimTags($arr2[0][6]));
	        if(empty($price[0])) {
	            $price = explode('-', $data['Hotel']['eaddress']);
	            $data['Hotel']['eaddress'] = '';
	        }
	        $data['Hotel']['price_l'] = $price[0];
	        $data['Hotel']['price_h'] = $price[1];
	        preg_match('/mailto:([^"]*)/', $arr1[0][7], $mail);
	        $data['Hotel']['email'] = $mail[1];
	        preg_match('/http:[^"]*/', $arr2[0][7], $website);
