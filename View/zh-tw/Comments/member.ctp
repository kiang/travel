...
<?php if ($offset === 0) { ?>
    ...
    <?php if (!empty($items)) { ?>
        <?php if ($offset === 0) { ?>
            <div class="<?php echo!empty($loginMember['id']) ? 'fields_c' : 'fields_1'; ?>">
                <dl class="list1" id="commentBlockMemberMain">
                <?php } ?>
                <?php
            ...
            <?php if ($offset === 0) { ?>
                <p><a class="dbtn dbtn3 fillet_all" href="#" id="commentBlockMemberMore">瀏覽更多內容 &gt;&gt;</a></p>
            </div>
        <?php } ?>
    <?php } ?>
    <?php if ($offset === 0) { ?>
    ...
    <?php if (!empty($loginMember['id'])) { ?>
                    ...
                    $('form#CommentMemberForm').submit(function() {
                        if(false === submitted) {
                            submitted = true;
                            $('#commentControlMessage').hide();
                            $.post('<?php echo $this->Html->url($url); ?>', $(this).serializeArray(), function(pageData) {
                                if(pageData == 'done') {
                                    $('div#commentsMemberView').load(wwwRoot + 'comments/member/<?php echo $memberId; ?>');
                                } else {
    ...
    <?php } ?>
                var commentBlockMemberOffset = <?php echo $offset; ?>;
                var previousResult = '';
                $('a#commentBlockMemberMore').click(function() {
                    commentBlockMemberOffset += 5;
                    $.get('<?php echo $this->Html->url('/comments/member/' . $memberId . '/'); ?>' + commentBlockMemberOffset, {}, function(result) {
                        if(previousResult === result) {
                            $('a#commentBlockMemberMore').hide();
                        } else {
                            $('#commentBlockMemberMain').append(result);
