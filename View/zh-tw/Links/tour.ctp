...
<?php if (!empty($items)) { ?>
    <?php if ($offset === 0) { ?>
        <div class="<?php echo!empty($loginMember['id']) ? 'fields_c' : 'fields_1'; ?>">
            <dl class="list1" id="linkBlockTourMain">
            <?php } ?>
            <?php
        ...
        <?php if ($offset === 0) { ?>
            <p><a class="dbtn dbtn3 fillet_all" href="#" id="linkBlockTourMore">瀏覽更多內容 &gt;&gt;</a></p>
        </div>
    <?php } ?>
<?php } ?>
<?php if ($offset === 0) { ?>
    ...
    <script type="text/javascript">
        //<![CDATA[
        $(function() {
            $('#rankingAtLink').load('<?php echo $this->Html->url('/ranks/add/rankingAtLink/Tour/' . $tourId); ?>');
    <?php if (!empty($loginMember['id'])) { ?>
                var submitted = false;
                ...
                $('form#LinkTourForm').submit(function() {
                    if(false === submitted) {
                        submitted = true;
                        $('#linkControlMessage').hide();
                        $.post(wwwRoot + 'links/add/Tour/<?php echo $tourId; ?>', $(this).serializeArray(), function(pageData) {
                            if(pageData == 'done') {
                                $('p.tourViewLinks').load(wwwRoot + 'links/tour/<?php echo $tourId; ?>');
                            } else {
    ...
    <?php } ?>
            var linkBlockTourOffset = <?php echo $offset; ?>;
            var previousResult = '';
            $('a#linkBlockTourMore').click(function() {
                linkBlockTourOffset += 5;
                $.get(wwwRoot + 'links/tour/<?php echo $tourId; ?>' + linkBlockTourOffset, {}, function(result) {
                    if(previousResult === result) {
                        $('a#linkBlockTourMore').hide();
                    } else {
                        $('#linkBlockTourMain').append(result);
