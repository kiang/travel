<?php if ($offset === 0): ?>
    ...
    <div class="fields_bg1" id="scheduleBlockGoodMain">
    <?php endif; ?>
    <?php
    if (!empty($items)) {
    ...
    <p><a class="dbtn dbtn3 fillet_all" href="#" id="scheduleBlockGoodMore">瀏覽更多內容 &gt;&gt;</a></p>
    <script type="text/javascript">
        <!--
        $(function() {
            var scheduleBlockGoodOffset = <?php echo $offset; ?>;
            var previousResult = '';
            $('a#scheduleBlockGoodMore').click(function() {
                scheduleBlockGoodOffset += 20;
                $.get('<?php echo $this->Html->url('/schedules/block_new/'); ?>' + scheduleBlockGoodOffset, {}, function(result) {
                    if(previousResult === result) {
                        $('a#scheduleBlockGoodMore').hide();
                    } else {
                        $('div#scheduleBlockGoodMain').append(result);
