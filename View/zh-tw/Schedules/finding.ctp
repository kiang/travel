<?php if (empty($offset)) { ?>
    ...
    echo $this->Html->link($item['Schedule']['title'], array('action' => 'view', $item['Schedule']['id']));
        ?></div>
                ...
                echo $this->element('icon', array('iconData' => $item['Schedule']['Member']));
                $genderClass = 'spot_XY';
                if (isset($item['Schedule']['Member']['gender']) && $item['Schedule']['Member']['gender'] === 'f') {
                    $genderClass = 'spot_XX';
                }
        ?></div>
    ...
    <div class="block"><a id="scheduleFindingMore" class="dbtn dbtn3 fillet_all" href="#">瀏覽更多內容 &gt;&gt;</a></div>
    <div class="clearfix"></div>
    <script type="text/javascript">
        <!--
        $(function() {
            var scheduleFindingOffset = <?php echo $offset; ?>;
            var previousResult = '';
            $('a#scheduleFindingMore').click(function() {
                scheduleFindingOffset += 10;
                $.get('<?php echo $this->Html->url($url); ?>/' + scheduleFindingOffset, {}, function(result) {
                    if(previousResult === result) {
                        $('a#scheduleFindingMore').hide();
                    } else {
                        $('div#scheduleFindingMain').append(result);
