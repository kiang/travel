<div id="schedulePageHotMain" class="fields_2box">
    <?php
    if (!empty($items)) {
...
<?php if ($offset === 0): ?>
    <div class="clearfix"></div>
    <div class="block"><a class="dbtn dbtn3 fillet_all" href="#" id="schedulePageHotMore">瀏覽更多內容 &gt;&gt;</a></div>
    <script type="text/javascript">
        <!--
        $(function() {
            var schedulePageHotOffset = <?php echo $offset; ?>;
            var previousResult = '';
            $('a#schedulePageHotMore').click(function() {
                schedulePageHotOffset += 20;
                $.get('<?php echo $this->Html->url('/schedules/page_hot/'); ?>' + schedulePageHotOffset, {}, function(result) {
                    if(previousResult === result) {
                        $('a#schedulePageHotMore').hide();
                    } else {
                        $('div#schedulePageHotMain').append(result);
