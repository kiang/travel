<div id="SubmitsAdminControlPage">
    ...
    <table class="table table-bordered" cellpadding="0" cellspacing="0" id="SubmitsListTable">
        ...
        echo ' / ' . $this->Html->link($item['Submit']['foreign_title'], array(
            ...
            ?></td>
                <td><?php echo ($item['Submit']['is_new'] == 1) ? '新增' : '編輯'; ?></td>
                <td><?php
                if ($accepted) {
            ...
            ?></td>
                <td>
                    <div class="btn-group">
                        ...
                        if (!$accepted) {
                            echo $this->Html->link('編輯', array('action' => 'edit', $item['Submit']['id']), array('class' => 'SubmitControl btn'));
                            echo $this->Html->link('接受', array('action' => 'accept', $item['Submit']['id']), array('class' => 'btn'));
                        }
                        echo $this->Html->link('刪除', array('action' => 'delete', $item['Submit']['id']), array('class' => 'btn'), '確定要刪除？');
                        ?>
                    </div>
                </td>
            </tr>
