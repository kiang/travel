<?php if ($offset === 0) { ?>
    <div class="TabBox">
        ...
        <div class="fields_3box" id="pointBlockHotMain">
        <?php } ?>
        <?php
        foreach ($items AS $item) {
            ...
            $icon = $this->Html->link($icon, '/members/view/' . $item['Comment']['member_id'], array(
                'title' => 'Browse ' . $item['Comment']['member_name'] . '\'s profile',
                'escape' => false,
                    ));
            $title = '';
            ...
            ?></div>
                    ...
                    <ul class="list1 float-l">
                        <?php if (!empty($loginMember['id'])) { ?>
                            <li><?php
                    echo $this->Html->link('Import', '/schedule_lines/push/Point/' . $item['Point']['id'], array(
                        ...
                        <?php } ?>
                        <li><?php
                    echo $this->Html->link('Clicks', '/points/view/' . $item['Point']['id'], array(
        ...
        <div class="clearfix"></div>
        <div class="block"><a class="dbtn dbtn3 fillet_all" href="#" id="pointBlockHotMore">More &gt;&gt;</a></div>
        <div class="clearfix"></div>
        <script type="text/javascript">
            <!--
            $(function() {
                var pointBlockHotOffset = <?php echo $offset; ?>;
                var previousResult = '';
                $('a#pointBlockHotMore').click(function() {
                    pointBlockHotOffset += 15;
                    $.get('<?php echo $this->Html->url('/points/block_hot/'); ?>' + pointBlockHotOffset, {}, function(result) {
                        if(previousResult === result) {
                            $('a#pointBlockHotMore').hide();
                        } else {
                            $('div#pointBlockHotMain').append(result);
