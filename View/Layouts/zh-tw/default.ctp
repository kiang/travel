    ...
    <body>
        ...
        ?></h1>
                        <ul id="Btns" class="float-r">
                            ...
                            $buttons = array(
                                array(
                                    '建行程', 'schedules/add', 'dbtn_route'
                                ), array(
                                    '找行程', 'schedules', 'dbtn_area'
                                ), array(
                                    '找地點', 'points', 'dbtn_point'
                                )
                            );
                            $buttonActived = false;
                    ...
                    <div class="block">
                        ...
                        <div class="spot overspots <?php echo $genderClass; ?>"><?php
                            if (!empty($loginMember['id'])) {
                                ...
                                echo $this->Html->link('訪客', '/members/login');
                            }
                            ?></div>
                        <?php
                        if (!empty($loginMember['id'])) {
                            echo $this->Html->link('<strong>登出帳戶</strong>', '/members/logout', array(
                                'escape' => false,
                            )) . ' | ';
                        }
                ...
                <?php
                echo $this->Html->link('<i class="icon-flag"></i> English', '/members/l/en-us', array('class' => 'btn span1 pull-right', 'escape' => false));
                ?>
                <div class="clearfix"></div>
                <hr class="line" />
                <div id="Cart">
        ...
        <script id="gmapToolBox" type="text/x-jquery-tmpl">
            <div style="position:absolute; background-color: #FFF; border: 1px solid black; padding: 5px; z-index: 2000;">
                <div align="right"><a href="#" id="close${mapId}"><i class="icon-remove"></i></a></div>
                <form onsubmit="return false;">
                    輸入地點住址來找尋座標(為了在地圖上展示)：<br />
                    <div class="control-group input-prepend">
                        <input type="text" id="text${mapId}" class="span5" onclick="$(this).select();" />
                        <a class="btn btn-primary" id="button${mapId}">找</a>
                    </div>
                </form>
                <div id="${mapId}" style="width: 500px; height: 400px;"></div>
                <div><input type="text" class="span5" id="address${mapId}" onclick="$(this).select();" /></div>
            </div>
        </script>
        <script id="gmapToolWindow" type="text/x-jquery-tmpl">
            <div>
                <b>座標：</b> ${lat}, ${lng}
                <br /><b>查詢：</b> ${address}
                <br /><b>標示：</b> ${formatted_address}
            </div>
        </script>
    </body>
</html>